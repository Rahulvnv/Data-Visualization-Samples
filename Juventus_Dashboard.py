...
st.title("Juventus Dashboard 2022/23")
...
if(selected_viz_type=="Player Report"):
    st.sidebar.header('Choose Report Type')
    options1=['Single Match Report','Full season Report']
    selected_player_report_type = st.sidebar.selectbox('Player Report Type', options1)
    if(selected_player_report_type=="Single Match Report"):
        ...
        bs_heatmap1 = pitch.bin_statistic_positional(hal["x"], hal["y"], positional='full', statistic='count',
                                                     normalize=True)
        hm = pitch.heatmap_positional(bs_heatmap1, ax=axs["pitch"][2], cmap='Reds', edgecolor="black")
        pitch.label_heatmap(bs_heatmap1, color="white", ha="center", va="center", backgroundcolor='0.5',
                            ax=axs["pitch"][2],
                            str_format='{:.0%}', fontsize=30)
...
if(selected_viz_type=="Match Report"):
    ...
    for i in range(len(lineupa)):
        ...
        try:
            ...
            pitch.annotate(lineupa[i], xy=(
                avgdfa[avgdfa["playerName"] == lineupa[i]]["x"] - 3, avgdfa[avgdfa["playerName"] == lineupa[i]]["y"]),
                           c='white', va='center', ha='center', ax=axs["pitch"][1][2], fontsize=22)
    axs["pitch"][0][0].set_title(team_plot_data["home_team"].unique()[0] + " Passes", color="white", fontsize=30)
    ...
    axs["pitch"][1][0].set_title(team_plot_data["away_team"].unique()[0] + " Passes", color="white", fontsize=30)
